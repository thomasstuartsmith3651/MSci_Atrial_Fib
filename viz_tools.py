...
from numpy import meshgrid # previously "from scipy import meshgrid" but deprecated. 
import os
...
def anim_2D(X, Y, L, pas_de_temps, pas_d_images, save = False, myzlim = (-0.15, 0.15)):
    ...
    # Save the result
    if save:
        writer = animation.FFMpegWriter(fps = 15, bitrate = 500, codec = "libx264", extra_args = ["-pix_fmt", "yuv420p"])
        # writer = animation.FFMpegWriter(fps = 24, bitrate = 10000, codec = "libx264", extra_args = ["-pix_fmt", "yuv420p"])
        anim.save('anim_2D.mp4',writer=writer)
...
def anim_2D_flat(X, Y, L, pas_de_temps, pas_d_images, save = False,  myzlim = (-0.15, 0.15)): # vmin, vmax
    ...
    if save:
        writer = animation.FFMpegWriter(fps = 15, bitrate = 500, codec = "libx264", extra_args = ["-pix_fmt", "yuv420p"])
        # writer = animation.FFMpegWriter(fps = 24, bitrate = 10000, codec = "libx264", extra_args = ["-pix_fmt", "yuv420p"])
        anim.save('anim_2D_flat.mp4',writer=writer)
